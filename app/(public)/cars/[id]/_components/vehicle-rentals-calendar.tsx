...
const VehicleRentalsCalendar = ({
	...
	return (
		<div className="container @container flex flex-col md:flex-row lg:justify-between w-full gap-8 p-6 sm:p-8">
				...
				<div className="py-2 rounded-lg shadow-lg bg-background">
					...
					<div className="grid grid-cols-7 gap-2">
						{days.map((day, dayIdx) => (
							<Link
								prefetch={false}
								href={`/cars/${car.id}?time=${time[0].hour}&date=${format(day, "yyyy-MM-dd")}`}
								scroll={false}
								key={day.getTime()}
